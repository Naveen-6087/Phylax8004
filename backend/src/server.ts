/**
 * Private Medical AI Server
 *
 * Main Express server combining:
 * - nilDB: Encrypted storage for medical conversations
  ...
  return paymentMiddleware(
    {
      ...
      'POST /api/chat/stream': {
        ...
        description: 'Private Medical AI Streaming Query',
      ...
      'POST /a2a': {
        ...
        description: 'Private Medical AI A2A Query',
        mimeType: 'application/json',
      },
    },
    x402Server
  ...
  try {
    ...
    app.post('/api/chat/free', async (req, res) => {
      try {
        ...
        try {
          await nildbService.storePrompt(currentSessionId, message);
          console.log('[SUCCESS] Prompt stored in nilDB');
        } catch (dbError) {
        ...
        const response = await nilaiService.processQuery(message, [], true);
        ...
        try {
          await nildbService.storeResponse(currentSessionId, response);
    ...
    app.listen(PORT, () => {
      console.log('\n========================================');
      console.log('Private Medical AI Server');
      console.log('========================================');
