 ...
 *
 * Registers the Private Medical AI agent on the ERC-8004 Identity Registry.
 *
 * Steps:
...
async function registerAgent() {
  ...
  const metadata: AgentMetadata = {
    ...
    description:
      'A privacy-preserving medical AI assistant powered by Nillion. ' +
      'Uses nilAI for secure LLM inference in TEE, nilDB for encrypted data storage, ' +
      'and x402 for pay-per-query payments. All medical conversations are encrypted ' +
  ...
  try {
    ...
    if (result.agentId) {
      ...
      console.log(`   https://www.8004scan.io/sepolia/agent/${result.agentId}`);

      // Update metadata with registration info
      metadata.registrations = [
        {
          agentId: parseInt(result.agentId, 10),
      ...
      console.log('\n[SAVED] Saved registration.json with agent ID');
    }
